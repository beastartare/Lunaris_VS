...
export async function getUsuariosPorTipo(): Promise<UsuarioTipo[]> {
  ...
  data?.forEach((usuario) => {
    switch (usuario.tipo_acesso_usuario) {
      ...
      case 1:
        contagem.astronomico++;
        break;

      case 2:
        contagem.meteorologico++;
        break;
    }
  });
  ...
  return [
    {
      name: "Comum",
      value: contagem.comum,
    },
    {
      name: "Pesquisador Astronômico",
      value: contagem.astronomico,
    },
    {
      name: "Pesquisador Meteorológico",
      value: contagem.meteorologico,
    },
  ];
}
  ...
  return [
    {
      ...
      Quantidade: astronomicos,
    },
    {
      categoria: "Meteorológico",
      Quantidade: meteorologicos,
    },
  ];
}
...
export async function getMateriaisFavoritos(): Promise<MaterialFavorito[]> {
  ...
  (data as MaterialRow[])?.forEach(
    (item) => {
      const titulo =
        item.material?.[0]?.titulo;
